...
class liver_3_classification_net(nn.Module):
    def __init__(self,num_classes=2,aux_loss=False,drop_path_rate=0.0,return_feature=False,**kwargs):
    ...
    def forward(self, x):#original_img,liver_img,tumor_img
        ...
        #     out1, out2 = self.cross_tumor_liver[0][i](out1, out2)
        feature = []
        feature.append(x_original[-1])
        feature.append(x_liver[-1])
        feature.append(x_tumor[-1])
        #这一步似乎没有必要,直接拼起来聚合特征就好，待考虑
        # x_tumor[0]=self.frist_tumor_stages(x_tumor[0])
        # x_liver[0] = self.frist_liver_stages(x_liver[0])
        # x_original[0] = self.frist_original_stages(x_original[0])
        ...
        # for i in range(B):
        #     image = feature[0][i].detach().cpu().numpy().transpose(1, 2, 0)
        #     if False:#是否所以通道相加
        #         image = image.sum(-1)
        #         fig = plt.figure()
        #         ax = fig.add_subplot(1, 1, 1)
        ...
        for i in range(3):
            down = self.downsample_layers[i](x)
            fuse = self.fuse_stagelayers[i](torch.cat((down, feature[i + 4]), dim=1))
            fuse = self.fuse_stages[i](fuse)
            x = fuse
        ...
        if self.aux_loss:
            head0 = self.head0(feature[3].mean([-2, -1]))
            head1 = self.head1(feature[4].mean([-2, -1]))
            head2 = self.head2(feature[5].mean([-2, -1]))
            head3 = self.head3(feature[6].mean([-2, -1]))
            #weighted_pred = torch.zeros_like(result)
            weighted_pred = []
            for i ,pre in enumerate([mulit_scale_head,head0,head1,head2,head3,aux_original,aux_liver,aux_tumor]):
                #weighted_pred += self.classifier_weight[i]*pre
            ...
            if self.return_feature:
                feature.append(x)
                return weighted_pred,feature
            return weighted_pred
            # return sum(weighted_pred)
#((checkpoint['state_dict']['classifier_weight']-0.125)*10).exp().softmax(0)
            #return [result,head0,head1,head2,head3,aux_original,aux_liver,aux_tumor]#,self.vote_weights
        else:
            return None
