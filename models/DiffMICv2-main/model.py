...
class ConditionalModel(nn.Module):
    def __init__(self, config, guidance=True):
        ...
        _arch_l = str(arch).lower()
        if _arch_l == 'medvit':
            from medvit_encoder import MedViTEncoder
            self.encoder_x = MedViTEncoder(feature_dim=feature_dim, config=config)
        elif _arch_l in ('sam', 'efficient_sam', 'efficient_sam_vits', 'efficient_sam_vitt'):
            self.encoder_x = SamEncoder(arch=arch, feature_dim=feature_dim, config=config)
        else:
            self.encoder_x = ResNetEncoder(arch=arch, feature_dim=feature_dim, config=config)
        self.norm = nn.BatchNorm1d(feature_dim)

        _local_arch = 'resnet18' if _arch_l == 'medvit' else arch
        self.encoder_x_l = ResNetEncoder(arch=_local_arch, feature_dim=feature_dim, config=config, local=True)
...
class ResNetEncoder(nn.Module):
    def __init__(self, arch='resnet18', feature_dim=128, config=None, local=False):
        ...
        self.f = []
        #print(arch)
        _use_pretrained = os.environ.get('DIFFMICV2_PRETRAINED', '0') == '1'
        if arch == 'resnet50':
            backbone = resnet50(pretrained=_use_pretrained)
            self.featdim = backbone.fc.weight.shape[1]
        elif arch == 'resnet18':
            backbone = resnet18(pretrained=_use_pretrained)
            self.featdim = backbone.fc.weight.shape[1]
