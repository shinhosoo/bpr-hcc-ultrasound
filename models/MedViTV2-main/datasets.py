...
def build_dataset(args):
    ...
    if args.dataset == 'Kvasir':
        ...
        return train_dataset, test_dataset, nb_classes
    elif args.dataset in ('lesion_binary', 'image_folder', 'custom'):
        train_path = getattr(args, 'train_path', None) or os.environ.get('MEDVITV2_TRAIN_PATH')
        val_path   = getattr(args, 'val_path', None)   or os.environ.get('MEDVITV2_VAL_PATH')
        if not train_path or not val_path:
            raise SystemExit("lesion_binary: --train-path 와 --val-path 또는 환경변수 MEDVITV2_TRAIN_PATH/VAL_PATH 필요")
        train_dataset = datasets.ImageFolder(root=train_path, transform=train_transform)
        test_dataset  = datasets.ImageFolder(root=val_path,   transform=test_transform)
        nb_classes = len(train_dataset.classes)
        print(f"[lesion_binary] train={len(train_dataset)} val={len(test_dataset)} classes={train_dataset.classes}")
        return train_dataset, test_dataset, nb_classes
    else:
...
def build_transform(args):
    ...
    t_train.append(transforms.AugMix(alpha= 0.4))          # disabled: no-aug setting
    #t_train.append(transforms.Lambda(lambda image: image.convert('RGB')))
    t_train.append(transforms.RandomHorizontalFlip(p=0.4)) # disabled: no-aug setting
    t_train.append(transforms.ToTensor())
    t_train.append(transforms.Normalize(mean=IMAGENET_MEAN, std=IMAGENET_STD))
    ...
    t_test.append(transforms.Normalize(mean=IMAGENET_MEAN, std=IMAGENET_STD))
