...
def main():
    ...
    # CODEx patch: accept launcher-provided YAML if the config file is not visible here.
    if os.path.exists(args.config):
        with open(args.config) as f:
            config_raw = f.read()
        print(f"[dcg-pretrain] config loaded: {args.config}")
    else:
        config_raw = os.environ.get("BPR_DCG_CONFIG_YAML", "")
        if not config_raw:
            raise FileNotFoundError(args.config)
        print(f"[dcg-pretrain] config file missing; using YAML passed from launcher: {args.config}")
    config = EasyDict(yaml.safe_load(config_raw))
    ...
    for epoch in range(epochs):
        ...
        for x, y in train_loader:
            ...
            y_fusion, y_global, y_local, patches, attns, attn_map = dcg(x)
            loss_ce = ce(y_fusion, y) + 0.5 * (ce(y_global, y) + ce(y_local, y))
